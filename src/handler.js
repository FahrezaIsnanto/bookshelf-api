...
const _requestValidation = ({name,readPage,pageCount},type)=>{
    messageType = type === 'add' ? 'menambahkan' : 'memperbarui';
    if(!name){
...
const addBookHandler = (request,h)=>{
    ...
    const requestValidation = _requestValidation(request.payload,'add');
    if(!requestValidation.status){
        const response = h.response(requestValidation.res);
        response.code(400);
        return response;
    }
...
const getAllBookHandler = ()=>{
    ...
    books.map((book)=>{
        ...
        resBooks.push({id,name,publisher});
    });
...
const editBookByIdHandler = (request, h) => {
    ...
    const requestValidation = _requestValidation(request.payload,'edit');
    if(!requestValidation.status){
        const response = h.response(requestValidation.res);
        response.code(400);
        return response;
